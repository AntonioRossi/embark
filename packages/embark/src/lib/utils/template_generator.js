...
class TemplateGenerator {
  ...
  generate(destinationFolder, name) {
    ...
    const monorepoRoot = this.insideMonorepo();
    if (monorepoRoot) {
      templateSpecifier = utils.joinPath(
        monorepoRoot, 'dapps/templates', this.templateName
      );
    } else {
  ...
  insideMonorepo() {
    if (this._monorepoRoot === undefined) {
      const maybeInsideMonorepo = fs.existsSync(
        fs.embarkPath('../../packages/embark')
      );
      let monorepoRoot = null;
      if (maybeInsideMonorepo) {
        const lernaJsonPath = findUp.sync('lerna.json', {cwd: fs.embarkPath()});
        if (lernaJsonPath) {
          monorepoRoot = utils.dirname(lernaJsonPath);
        }
      }
      this._monorepoRoot = monorepoRoot;
    }
    return this._monorepoRoot;
  }
